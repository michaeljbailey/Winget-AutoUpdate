...
function Invoke-AsCurrentUser {
    ...
    if ($encodedcommand.length -gt $MaxLength -and $CacheToDisk -eq $false) {
        Write-ToLog "The encoded script is longer than the command line parameter limit. Please execute the script with the -CacheToDisk option."
        return
    }
    if ($UseMicrosoftPowerShell -and -not (Test-Path -Path "$env:ProgramFiles\PowerShell\7\pwsh.exe"))
    {
        Write-ToLog "Not able to find Microsoft PowerShell v7 (pwsh.exe). Ensure that it is installed on this system"
        return
    }
    ...
    if (-not $privs -or ($privs -band [RunAsUser.PrivilegeAttributes]::Disabled)) {
        Write-ToLog "Not running with correct privilege. You must run this script as system or have the SeDelegateSessionUserImpersonatePrivilege token."
        return
    }
    else {
        try {
            ...
            Write-ToLog "Could not execute as currently logged on user: $($_.Exception.Message)" -Exception $_.Exception
            return
        }
    }
}

Function Get-UserPromptYesNo ($Message, $Title) {
    $scriptBlock = {
        Add-Type -AssemblyName "System.Windows.Forms"
        # Do not remove double quotes around the $message and $title, it will break ExpandStringVariables functionality.
        return [System.Windows.Forms.MessageBox]::Show("$message", "$title", [System.Windows.Forms.MessageBoxButtons]::YesNo , [System.Windows.Forms.MessageBoxIcon]::Question, [System.Windows.Forms.MessageBoxDefaultButton]::Button2, [System.Windows.Forms.MessageBoxOptions]::ServiceNotification)
    }
    
    Write-ToLog "Asking user Yes/No prompt"
    $answer = Invoke-AsCurrentUser -CaptureOutput -ScriptBlock $scriptBlock -ExpandStringVariables
    Write-ToLog "-> User Response = '$answer'"
